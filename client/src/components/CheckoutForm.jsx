...
export default function CheckoutForm({ onCloseAll }) {
  ...
  const handleSendWhatsApp = () => {
    if (!phone.trim()) {
      alert("Нужно указать номер телефона.");
      return;
    }
    ...
    lines.push("*Заказ:*");
    items.forEach(it => {
      const line = `• ${it.name} - ${it.qty} x ${it.price}р = ${
    ...
    lines.push("");
    lines.push(`*Итого к оплате:* ${calculateTotal()}р`);
    lines.push(
      `*Способ оплаты:* ${
        payment === "transfer" ? "Переводом" : "Наличными при получении"
      }`
    );
    lines.push("");
    lines.push("*Мои данные:*");
    lines.push(`Имя: ${name || "-"}`);
    lines.push(`Телефон: ${phone}`);
    lines.push(`Адрес: ${address || "-"}`);
    lines.push(`Комментарий: ${comment || "-"}`);

    // ✅ финальная сборка текста
    const text = lines.join("\n");

    const phoneDigits = RESTAURANT_PHONE.replace(/\D/g, "");
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <label className="flex flex-col sm:col-span-2">
          ...
          <textarea
            ...
            className="mt-1 p-2 rounded border"
            placeholder="Пожалуйста, позвоните за час..."
          />
        </label>
        <div className="sm:col-span-2">
          <div className="text-sm mb-1">Способ оплаты</div>
